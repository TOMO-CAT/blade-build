    ...
    if if_docker_container_exist(docker_container):
        ...
        docker_run_cmd = (
            ...
            + [
                ...
                # @see https://stackoverflow.com/questions/63898430/how-can-i-access-a-service-running-on-wsl2-from-inside-a-docker-container
                # 这种做法会在 docker 容器里的 `/etc/hosts` 里新增一条 `host.docker.internal` 记录
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--arch",
        type=str,
        default="ubuntu2204_cuda11.8",
        choices=["ubuntu2204_cuda11.8"],
        help="architecture",
    )
    parser.add_argument(
        "-C",
    ...
    docker_image = f"blade-{args.arch}:latest"
    docker_container = f"blade-{args.arch}-container"
