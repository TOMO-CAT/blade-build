...
class ThriftParser(object):
    ...
    def _parse_file(self):
        for line in open(self.path):
            ...
            matched = re.match(
                "(const|struct|service|enum|exception) ([0-9_a-zA-Z]+)", line
            )
            if not matched:
                continue
...
class FBThriftHelper(ThriftParser):
    def get_generated_cpp_files(self):
        files = [
            "gen-cpp/%s_constants.cpp" % self.thrift_name,
    ...
    def get_generated_cpp2_files(self):
        files = [
            "gen-cpp2/%s_constants.cpp" % self.thrift_name,
...
class ThriftHelper(ThriftParser):
    ...
    def get_generated_cpp_files(self):
        ...
        files = ["%s_types.cpp" % thrift_name, "%s_types.h" % thrift_name]
        if self.has_constants:
            files.append(
                "%s_constants.cpp" % thrift_name, "%s_constants.h" % thrift_name
            )
