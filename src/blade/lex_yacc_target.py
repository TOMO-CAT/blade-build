...
class LexYaccLibrary(CcTarget):
    ...
    def __init__(
        ...
        if (
            ...
            self.error(
                '"lex_yacc_library.srcs"  must be a pair of [lex_file, yacc_file]'
            )
    ...
    def _lex_rules(self, source, implicit_deps, vars):
        ...
        self.generate_build(
            "lex", cc_path, inputs=input, implicit_deps=implicit_deps, variables=vars
        )
        return cc, cc_path
    ...
    def _yacc_rules(self, source, rule, vars):
        ...
        input = self._source_file_path(source)
        self.generate_build(
            "yacc", cc_path, inputs=input, implicit_outputs=h_path, variables=vars
        )
        return cc, cc_path, h_path

    def generate(self):
        lex_file, yacc_file = self.srcs
        yacc_cc, yacc_cc_path, yacc_h_path = self._yacc_rules(
            yacc_file, "yacc", vars=self._yacc_vars()
        )
        lex_cc, lex_cc_path = self._lex_rules(
            lex_file, implicit_deps=[yacc_cc_path], vars=self._lex_vars()
        )
        objs = self._generated_cc_objects(
            [lex_cc, yacc_cc], generated_headers=self.attr["generated_hdrs"]
        )
