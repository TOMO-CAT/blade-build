    ...
    with open(header, "w") as h, open(source, "w") as c:
        ...
        c.write(
            textwrap.dedent(
                ...
                const struct BladeResourceEntry {1}[] = {{"""
            ).format(header, index_name)
        )
        ...
        c.write(
            textwrap.dedent(
                ...
                const unsigned {0}_len = {1};"""
            ).format(index_name, len(sources))
        )
        h.write(
            textwrap.dedent(
                """\
...
def generate_one_jar(onejar, main_class, bootjar, args):
    ...
    onejar.write(main_jar, os.path.join("main", os.path.basename(main_jar)))
    for dep in jars:
...
def main():
    ...
    try:
        ...
        console.error(
            "Blade build tool %s error: %s %s" % (name, str(e), traceback.format_exc())
        )
        exit_code = 1
    finally:
        if exit_code:
