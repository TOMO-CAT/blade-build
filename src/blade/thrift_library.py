...
class ThriftLibrary(CcTarget):
    ...
    def __init__(
        self, name, srcs, deps, visibility, tags, optimize, deprecated, kwargs
    ):
    ...
    def generate(self):
        ...
        for src in self.srcs:
            thrift_files = self._thrift_gen_cpp_files(src)
            self.generate_build(
                "thrift", thrift_files, inputs=self._source_file_path(src)
            )
        ...
        objs = self._generated_cc_objects(
            sources, generated_headers=self.attr["generated_hdrs"]
        )
