...
class Workspace(object):
    ...
    def setup_build_dir(self):
        ...
        build_dir = s.substitute(
            bits=self.__options.bits, profile=self.__options.profile
        )
    ...
    def lock(self):
        ...
        lock_file_fd, ret_code = util.lock_file(
            os.path.join(self.__build_dir, _BUILDING_LOCK_FILE)
        )
        if lock_file_fd == -1:
            if ret_code == errno.EAGAIN:
                console.fatal(
                    "There is already an active building in current workspace."
                )
            else:
