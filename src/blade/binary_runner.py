...
class BinaryRunner(object):
    ...
    def __check_test_data_dest(self, target, dest, dest_list):
        ...
        for item in dest_list:
            ...
            if long_path.startswith(short_path) and long_path[len(short_path)] == "/":
                target.error(
                    '"%s" could not exist with "%s" in testdata' % (dest, item)
                )
    ...
    def _prepare_shared_libraries(self, target, runfiles_dir):
        ...
        os.symlink(
            os.path.abspath(self.build_dir), os.path.join(runfiles_dir, build_dir_name)
        )
        ...
        for soname, full_path in self._get_shared_libraries_with_soname(target):
            ...
            if os.path.lexists(dst):
                console.warning(
                    "Trying to make duplicate symlink for shared library:\n"
                    "%s -> %s\n"
                    "%s -> %s already exists\n"
                    "skipped, should check duplicate prebuilt "
                    "libraries" % (dst, src, dst, os.path.realpath(dst))
                )
                continue
            os.symlink(src, dst)
    ...
    def _prepare_test_data(self, target):
        ...
        for i in target.attr["testdata"]:
            ...
            if ".." in src:
                target.warning(
                    "Relative path is not allowed in testdata. Ignored %s." % src
                )
                continue
            if src.startswith("//"):
                src = src[2:]
            ...
            if os.path.exists(dest_path):
                target.warning(
                    '"%s" already existed, could not prepare testdata.' % dest
                )
                continue
            try:
    ...
    def _prepare_extra_test_data(self, target):
        ...
        testdata = os.path.join(
            self.build_dir, target.path, "%s.testdata" % target.name
        )
