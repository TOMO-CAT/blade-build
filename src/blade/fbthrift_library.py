...
class FBThriftLibrary(CcTarget):
    ...
    def __init__(
        self, name, srcs, deps, optimize, visibility, tags, deprecated, kwargs
    ):
        ...
        for src in srcs:
            self.fbthrift_helpers[src] = FBThriftHelper(os.path.join(self.path, src))

    def _thrift_gen_cpp_files(self, src):
        """Get the c++ files generated from thrift file."""
        return [
            self._target_file_path(f)
