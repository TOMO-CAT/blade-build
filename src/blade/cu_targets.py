...
class CuTarget(CcTarget):
    ...
    def _get_cuda_path(self, cuda_path):
        ...
        if cuda_path and not cuda_path.startswith("//"):
            console.fatal(
                "targets: %s cuda_path %s should start with //"
    ...
    def _get_cu_vars(self):
        ...
        vars["cuflags"] += " " + " ".join(["-Xcompiler %s" % flag for flag in cppflags])
...
class CuBinary(CuTarget):
    ...
    def generate(self):
        ...
        # Don't generate library file for header only library.
        self._cuda_binary(
            objs, inclusion_check_result, self.attr.get("dynamic_link", False)
        )
