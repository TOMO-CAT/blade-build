...
class HELP(object):
    ...
    run_unrepaired_tests = "Whether run unrepaired (no change after previous failure) tests during incremental test"
    ...
    fat_jar_compression_level = (
        "Fat jar compress level, must between 0 (store only) and 9 (max but slow)"
    )
    maven_download_concurrency = (
