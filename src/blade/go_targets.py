...
class GoTarget(Target):
    ...
    def __init__(self, name, type, srcs, deps, extra_goflags, visibility, tags, kwargs):
        """Init the go target."""
    ...
    def _set_go_package(self):
        ...
        if len(dirs) != 1:
            self.error(
                "Go sources belonging to the same package should be in the same "
        ...
        if go_module_enabled and not go_module_relpath:
            ...
            self.attr["go_package"] = os.path.relpath(
                self.path, os.path.join(go_home, "src")
            )
...
class GoTest(GoTarget):
    ...
    def __init__(
        self, name, srcs, deps, visibility, tags, testdata, extra_goflags, kwargs
    ):
        super().__init__(
            name=name,
            type="go_test",
...
def go_library(
    name, srcs, deps=[], extra_goflags=None, visibility=None, tags=[], **kwargs
):
...
def go_binary(
    name, srcs, deps=[], visibility=None, tags=[], extra_goflags=None, **kwargs
):
    build_manager.instance.register_target(
        GoBinary(
            name=name,
...
def go_package(name, deps=[], testdata=[], visibility=None, extra_goflags=None):
