...
class PackageTarget(Target):
    ...
    def __init__(self, name, srcs, deps, visibility, tags, type, out, shell, kwargs):
        ...
        if type not in _package_types:
            self.error(
                "Invalid type %s. Types supported by the package are %s"
    ...
    def _get_source_path(self, src, dst):
        ...
        if ".." in src or ".." in dst:
            self.error(
                "Invalid src (%s, %s). Relative path is not allowed." % (src, dst)
            )
