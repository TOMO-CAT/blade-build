...
from blade import console, constants
from blade.toolchain import BuildArchitecture, ToolChain
...
class CommandLineParser(object):
    ...
    def parse(self, argv):
        ...
        for t in targets:
            if t.startswith("-"):
                console.fatal(
                    "Unrecognized option %s, use blade [action] "
    ...
    def _check_plat_and_profile_options(self, options, targets):
        ...
        if not m:
            ...
            if options.arch is None:
                console.fatal(
                    '"-m%s" is not supported by the architecture %s'
                    % (m, compiler_arch)
                )
    ...
    def __add_plat_profile_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default="",
            help=(
                "Generate code for a 32-bit(-m32) or "
    ...
    def __add_build_actions_arguments(self, parser):
        ...
        parser.add_argument(
            "--backend-builder-options",
            dest="backend_builder_options",
            metavar="OPTIONS",
            help="Specifies extra backend builder options, for debug purpose",
        )

        parser.add_argument(
            "-j", "--jobs", dest="build_jobs", type=int, help=constants.HELP.build_jobs
        )
        ...
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="Do not build the test targets",
        )
        ...
        parser.add_argument(
            "--show-builds-slower-than",
            dest="show_builds_slower_than",
            metavar="SECONDS",
            type=float,
    ...
    def _add_test_arguments(self, parser):
        ...
        parser.add_argument(
            "--run-unrepaired-tests",
            dest="run_unrepaired_tests",
            action="store_true",
    ...
    def _add_common_arguments(self, *parsers):
        for parser in parsers:
            ...
            parser.add_argument(
                ...
                dest="stop_after",
                type=str,
                choices=["load", "analyze", "generate", "build", "all"],
                default="all",
            ...
            parser.add_argument(
                ...
                dest="fat_jar_compression_level",
                type=str,
    ...
    def _build_arg_parser(self):
        ...
        # subcommands
        build_parser = sub_parser.add_parser("build", help="Build specified targets")
        ...
        clean_parser = sub_parser.add_parser(
            "clean", help="Remove all blade-created output"
        )

        query_parser = sub_parser.add_parser(
            "query", help="Execute a dependency graph query"
        )

        dump_parser = sub_parser.add_parser(
            "dump", help="Dump specified internal information"
        )
