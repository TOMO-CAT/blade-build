...
class ShellTest(Target):
    ...
    def __init__(self, name, srcs, deps, visibility, tags, testdata, kwargs):
    ...
    def _process_test_data(self, testdata):
        ...
        for td in testdata:
            if isinstance(td, tuple):
                ...
                self.error(
                    "Invalid testdata %s. Test data should be either str or tuple." % td
                )
                continue
...
def sh_test(
    name=None, srcs=None, deps=[], visibility=None, tags=[], testdata=[], **kwargs
):
