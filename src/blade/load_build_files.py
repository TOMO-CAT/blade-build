...
def glob(include, exclude=None, excludes=None, allow_empty=False):
    ...
    if excludes:
        console.diagnose(
            source_loc, severity, '"excludes" is deprecated, use "exclude" instead'
        )
    ...
    def includes_iterator():
        ...
        for pattern in include:
            if not pattern:
                console.diagnose(
                    source_loc, "error", '"glob": Empty pattern is not allowed'
                )
                continue
...
def _get_globals_for_build_file(source_dir):
    ...
    if global_config.get("restricted_dsl") and source_dir not in global_config.get(
        "unrestricted_dsl_dirs"
    ):
...
def include(name):
    ...
    if not os.path.isfile(full_path):
        console.diagnose(
            _current_source_location(), "error", 'File "%s" does not exist' % name
        )
        return
...
def load(name, *symbols, **aliases):
    ...
    if not symbols and not aliases:
        console.diagnose(
            src_loc, "error", "The symbols to be imported must be explicitly declared"
        )
    ...
    def error(symbol):
        console.diagnose(
            src_loc, "error", '"%s" is not defined in "%s"' % (symbol, name)
        )
    ...
    if "*" in symbols:
        ...
        if len(symbols) != 1:
            console.diagnose(
                src_loc, "error", "wildcard import can't coexist with named imports"
            )
            return
    ...
    try:
        ...
        if os.path.isfile(build_file):
            try:
                ...
                console.fatal(
                    "Parse error in %s\n%s" % (build_file, traceback.format_exc())
                )
        else:
...
def load_targets(target_ids, excluded_targets, blade):
    ...
    excluded_targets, excluded_dirs, excluded_trees = _parse_excluded_targets(
        excluded_targets
    )
    # targets specified in command line
    # starting dirs mentioned in command line
    direct_targets, starting_dirs = _expand_target_patterns(
        blade, target_ids, excluded_trees
    )
    ...
    command_targets = _load_starting_build_files(
        blade, starting_dirs, processed_dirs, filter_function
    )
...
def _expand_target_patterns(blade, target_ids, excluded_trees):
    ...
    for target_id in target_ids:
        ...
        if skip_file:
            console.warning(
                '"%s" is under skipped directory due to "%s", ignored'
                % (target_id, skip_file)
            )
            continue
...
def _load_related_build_files(blade, command_targets, processed_dirs):
    ...
    while cited_targets:
        ...
        if skip_file:
            ...
            dependent.error(
                '"%s" is under skipped directory due to "%s"' % (target_id, skip_file)
            )
            continue
