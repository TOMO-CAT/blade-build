...
_color_enabled = sys.stdout.isatty() and os.environ.get("TERM") not in ("emacs", "dumb")
...
def _need_refresh_pgogress_bar(progress, current, now):
    return (
        now - _last_progress_time >= _PROGRESS_REFRESH_INTERVAL
        and _last_progress_value != current
    )
...
def diagnose(source_location, severity, message):
    ...
    globals()[severity](
        "%s: %s: %s" % (source_location, severity, message), prefix=False
    )
