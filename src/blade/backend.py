...
_INCLUSION_STACK_SPLITTER = (
    ...
    r"""!/^\.+ / && (!stop || (!/Multiple include guards may be useful for:/ && !/^[a-zA-Z0-9\.\/\+_-]+$$/ )) {print $$0 > "/dev/stderr"}"""  # Maybe error messages
    r"'"
)
...
class _NinjaFileHeaderGenerator(object):
    ...
    def generate_rule(
        ...
        if description:
            self._add_line(
                "  description = %s" % console.colored(description, "dimpurple")
            )
        if depfile:
    ...
    def generate_common_rules(self):
        self.generate_rule(
            name="copy", command="cp -f ${in} ${out}", description="COPY ${in} ${out}"
        )
    ...
    def _hdrs_command(self, cc, flags, cppflags, includes):
        ...
        args = (
            " -o /dev/null -E -MMD -MF ${out}.d %s %s -w ${cppflags} %s ${includes} ${in} "
        ...
        return (
            "export LC_ALL=C; %s || %s; ec=$$?; %s ${out}.err > ${out}; "
            "rm -f ${out}.err; exit $$ec"
        ) % (cmd1, cmd2, _INCLUSION_STACK_SPLITTER)
    ...
    def _generate_cc_link_rules(self, ld, linkflags):
        self._add_line(
            "linkflags = %s" % " ".join(config.get_item("cc_config", "linkflags"))
        )
        ...
        if go_home and go:
            ...
            if go_module_enabled:
                ...
                if go_module_relpath:
                    ...
                    out_relative = os.path.join(
                        os.path.relpath("./", go_module_relpath), ""
                    )
            else:
            ...
            self.generate_rule(
                name="gocommand",
                command="%s build -o %s${out} ${extra_goflags} ${package}"
                % (prefix, out_relative),
            ...
            self.generate_rule(
                name="gotest",
                command="%s test -c -o %s${out} ${extra_goflags} ${package}"
                % (prefix, out_relative),
    ...
    def generate_cuda_rules(self):
        ...
        link_args = (
            "-o ${out} ${includes} ${cppflags} %s ${target_linkflags} ${extra_linkflags} ${in}"
            % (" ".join(cuda_linkflags))
        )
        self.generate_rule(
            name="cudalink",
