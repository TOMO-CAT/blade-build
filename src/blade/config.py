...
class BladeConfig(object):
    ...
    def __init__(self):
        ...
        self.configs = {
            "global_config": {
                ...
                "test_related_envs__help__": "Environment variables which need to see whether changed before incremental "
                ...
                "glob_error_severity__help__": "The severity of glob error, can be %s"
            ...
            "cc_config": {
                ...
                "library to which the header file belongs, can be %s"
                % constants.SEVERITIES,
    ...
    def info(self, msg):
        console.info(
            "%s: info: %s" % (source_location(self.current_file_name), msg),
            prefix=False,
        )

    def warning(self, msg):
        console.warning(
            "%s: warning: %s" % (source_location(self.current_file_name), msg),
            prefix=False,
        )

    def error(self, msg):
        console.error(
            "%s: error: %s" % (source_location(self.current_file_name), msg),
            prefix=False,
        )
    ...
    def _append_config(self, section_name, section, append):
        ...
        self.warning(
            '"append" is deprecated, please use the "append_" prefix to append'
        )
        ...
        for name, value in user_config.items():
            ...
            if name.startswith("append_"):
                ...
                if item_name in section:
                    self._append_item_value(
                        section, name, item_name, value, user_config
                    )
                    continue
            if name.startswith("prepend_"):
                item_name = name[len("prepend_") :]
                if item_name in section:
                    self._prepend_item_value(
                        section, name, item_name, value, user_config
                    )
                    continue
    ...
    def _assign_item_value(self, section, name, value):
        ...
        if isinstance(section[name], list):
            ...
            self.error(
                'Incorrect type for "%s", expect "%s", actual "%s"'
...
def _check_kwarg_enum_value(kwargs, name, valid_values):
    ...
    if value is not None and value not in valid_values:
        _blade_config.error(
            'Invalid config item "%s" value "%s", can only be in %s'
...
def _check_test_related_envs(kwargs):
    for name in kwargs.get("test_related_envs", []):
        try:
            ...
            _blade_config.error(
                '"global_config.test_related_envs": Invalid env name or regex "%s", %s'
                % (name, e)
            )
...
def global_config(append=None, **kwargs):
    ...
    _check_kwarg_enum_value(
        kwargs, "duplicated_source_action", _DUPLICATED_SOURCE_ACTION_VALUES
    )
    debug_info_levels = _blade_config.get_section("cc_config")[
        "debug_info_levels"
    ].keys()
...
def protoc_plugin(**kwargs):
    """protoc_plugin."""
    from blade.proto_library_target import (  # pylint: disable=import-outside-toplevel
        ProtocPlugin,
    )
