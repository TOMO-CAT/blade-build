...
TestJob = namedtuple(
    "TestJob", ["reason", "binary_md5", "testdata_md5", "env_md5", "args"]
)
TestHistoryItem = namedtuple(
    "TestHistoryItem",
    [
        "job",  # TestJob
        "first_fail_time",
...
class TestRunner(binary_runner.BinaryRunner):
    ...
    def _merge_passed_run_results_to_history(self, run_results):
        ...
        for key, run_result in iteritems(run_results):
            ...
            history_items[key] = TestHistoryItem(
                self.test_jobs[key], first_fail_time=0, fail_count=0, result=run_result
            )
    ...
    def _get_test_target_md5sum(self, target):
        ...
        if target.attr.get("dynamic_link"):
            for dep in self._build_targets[target.key].expanded_deps:
                ...
                if "cc_library" in dep_target.type:
                    ...
                    lib_path = os.path.join(self.build_dir, dep_target.path, lib_name)
        ...
        for i in target.attr["testdata"]:
            ...
            if data_target.startswith("//"):
                ...
                data_target_path = os.path.abspath("%s/%s" % (target.path, data_target))
    ...
    def _collect_test_jobs(self):
        ...
        self.unrepaired_tests.sort(
            key=lambda x: self.test_history["items"][x].first_fail_time, reverse=True
        )
    ...
    def _is_full_success(self, passed_run_results):
        return len(passed_run_results) == len(self.test_jobs) + len(
            self.unrepaired_tests
        )
    ...
    def _show_run_results(self, run_results, is_error=False):
        ...
        for key, costtime, reason, result in tests:
            output_function(
                "  %s triggered by %s, exit(%s), cost %.2f s"
    ...
    def _show_unrepaired_results(self):
        ...
        for key in self.unrepaired_tests:
            test = items[key]
            first_fail_time = time.strftime(
                "%F %T %A", time.localtime(test.first_fail_time)
            )
            duration = datetime.timedelta(
                seconds=int(time.time() - test.first_fail_time)
            )
            console.error(
                "  %s: exit(%s), retry %s times, since %s, duration %s"
                % (
                    key,
    ...
    def _show_tests_summary(self, passed_run_results, failed_run_results):
        ...
        if self.unchanged_tests:
            console.info(
                "Skip %d unchanged tests when doing incremental test."
        ...
        total = (
            len(self.test_jobs) + len(self.unrepaired_tests) + len(self.unchanged_tests)
        )
